...
class DeliveryOrdersMapController extends GetxController {
  ...
  void checkGPS() async {
    deliveryMarker = await createMarkerFromAssets('assets/img/fumao.png');
  ...
  void updateLocation() async {
    try{
      ...
      addMarker(
          'delivery',
          position?.latitude ?? -8.11599,
          position?.longitude ?? -79.02998,
          'Tu posicion',
          '',
          deliveryMarker!
      );
      ...
      addMarker(
          'home',
          order.address?.lat ?? -8.11599,
          order.address?.lng ?? -79.02998,
          'Lugar de entrega',
          '',
          homeMarker!
      );
      ...
      LatLng to = LatLng(order.address?.lat ?? -8.11599, order.address?.lng ?? -79.02998);
